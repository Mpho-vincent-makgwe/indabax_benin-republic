...
const LeaderDetail = () => {
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
        <div className="text-center">
          <h1 className="text-4xl font-bold mb-4">{t('leaderDetail.notFound.title')}</h1>
          <p className="text-xl">{t('leaderDetail.notFound.message', { leaderId })}</p>
          <p className="text-lg mb-4">{t('leaderDetail.notFound.availableIds')}: {leaders.map(l => l.id).join(', ')}</p>
          <Link 
            to="/" 
  ...
  // Get translated leader data
const translatedLeader = {
  ...leader,
  role: t(`leaders.${leader.id}.role`),
  bio: t(`leaders.${leader.id}.bio`),
  responsibilities: t(`leaders.${leader.id}.responsibilities`, { returnObjects: true })
};
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="md:w-2/3">
              ...
              <div className="flex items-center mb-4">
                ...
                <span className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>{translatedLeader.country}</span>
              </div>
              ...
              <div className="flex items-center mb-6">
                ...
                <span className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>{translatedLeader.organization}</span>
              </div>
              
              <a
                href={translatedLeader.contact}
                target="_blank"
              ...
              >
                ...
                {t('common.contactPrefix')} {translatedLeader.name.split(' ')[0]}
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2">
            <h2 className={`text-2xl font-bold mb-6 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              {t('leaderDetail.biography')}
            </h2>
            <p className={`text-lg mb-6 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
              {translatedLeader.bio}
            </p>

            {/* Responsibilities Section */}
            {translatedLeader.responsibilities && translatedLeader.responsibilities.length > 0 && (
                <div className="mb-12">
                  <h2 className={`text-2xl font-bold mb-6 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                    {t('leaderDetail.responsibilities')}
                  </h2>
                  <div className={`p-6 rounded-xl ${theme === 'dark' ? 'bg-gray-800' : 'bg-green-50'}`}>
                    <ul className="grid sm:grid-cols-2 gap-4">
                      {translatedLeader.responsibilities.map((responsibility, index) => (
          ...
          <div>
            <div className={`sticky top-6 p-6 rounded-xl ${theme === 'dark' ? 'bg-black border border-white shadow-lg shadow-white/10' : 'bg-white'} shadow-md`}>
              ...
              <div className="space-y-4">
                <div>
                  <h4 className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{t('leaderDetail.role')}</h4>
                  <p className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>{translatedLeader.role}</p>
                </div>
                <div>
                  <h4 className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{t('leaderDetail.organization')}</h4>
                  <p className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>{translatedLeader.organization}</p>
                </div>
                <div>
                  <h4 className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{t('leaderDetail.country')}</h4>
                  <p className={theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}>{translatedLeader.country}</p>
                </div>
                <div>
                  <h4 className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>{t('common.contact')}</h4>
                  <a 
                    href={translatedLeader.contact} 
                    target="_blank" 
                    rel="noopener noreferrer"
                  ...
                  >
                    {translatedLeader.contact.startsWith('mailto:') 
                      ? translatedLeader.contact.replace('mailto:', '') 
                      : translatedLeader.contact}
                  </a>
                </div>
              </div>
            </div>
