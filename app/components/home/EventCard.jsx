...
const EventCard = ({ event, timers }) => {
  ...
  return (
    <div className="h-full flex flex-col"> {/* Added flex container */}
      <div className={`group relative overflow-hidden rounded-t-xl border border-transparent hover:border-green-400/30 transition-all duration-300 ${theme === 'dark' ? 'bg-gray-900/80' : 'bg-white'} backdrop-blur-sm shadow-lg hover:shadow-green-400/10 flex-grow`}>
      ...
      {/* Button outside the main card but still part of the flex column */}
      <Link 
  to={`/events/${event.id}`} 
  className={`mt-2 w-full py-2.5 flex items-center justify-center ${
    theme === 'dark' 
      ? 'bg-gray-50 hover:bg-gray-100 text-gray-900' 
      : 'bg-black hover:bg-gray-900 text-white'
  } text-sm font-semibold rounded-b-lg transition-all duration-300 shadow-lg hover:shadow-lg relative overflow-hidden group`}
>
